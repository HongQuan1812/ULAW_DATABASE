...
from Helper.cache import get_config
...
class NguoiBaoCaoValidator:
    ...
    def __init__(
        ...
        config_path: str = "../OrmModels_Definition/common_config.json"   # ✅ correct default
    ):
        self.db = db
...
# Setup Section
#----------------------------------
def get_nguoi_baocao_validator(db: AsyncSession = Depends(db_manager.get_db)) -> NguoiBaoCaoValidator:
    """Factory function to create NguoiBaoCaoValidator."""
    return NguoiBaoCaoValidator(db=db)  # Pass the actual session
...
def get_baocao_validator(db: AsyncSession = Depends(db_manager.get_db)) -> BaoCaoValidator:
    ...
    return BaoCaoValidator(db=db)  # Pass the actual session