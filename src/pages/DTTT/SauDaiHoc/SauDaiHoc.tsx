...
import TrainingMajorPicker from '@/components/MajorPicker/TrainingMajorPicker';
...
const SauDaiHoc: React.FC = () => {
  const trainingSystemType = 'SauDaiHoc';
  ...
  const [highStudyFeeFile, setHighStudyFeeFile] = useState<any[]>([]);
  ...
  const highStudyDegree = Form.useWatch(['highStudyInfor', 'highStudyDegree'], form);
  const educationType = Form.useWatch('educationType', form);
  ...
  const fetchAllMajorFromLocalJson = async () => {
    try {
      const response = await axios.get('/after_uni_major.json');
      if (response.data) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const payloadToSend = {
        ...
        highStudyInfor: {
          highStudyDegree: values.highStudyInfor?.highStudyDegree || '',
          highStudyGraduateMajor: values.highStudyInfor?.highStudyGraduateMajor || '',
          highStudyUniversity: values.highStudyInfor?.highStudyUniversity || '',
          highStudyDate: values.highStudyInfor?.highStudyDate || '',
          highStudyTrainingMajor: values.highStudyInfor?.highStudyTrainingMajor?.majorCode || '',
          highStudyDegreeFile: values.highStudyInfor?.highStudyDegreeFile || '',
          highStudyTranscript: values.highStudyInfor?.highStudyTranscript || '',
          highStudyApplication: values.highStudyInfor?.highStudyApplication || '',
          highStudyBackground: values.highStudyInfor?.highStudyBackground || '',
          highStudyReseachExperience: values.highStudyInfor?.highStudyReseachExperience || '',
          highStudyReseachProposal: values.highStudyInfor?.highStudyReseachProposal || '',
          highStudyPlan: values.highStudyInfor?.highStudyPlan || '',
          highStudyRecommendationLetter: values.highStudyInfor?.highStudyRecommendationLetter || '',
          highStudyLetterForStudy: values.highStudyInfor?.highStudyLetterForStudy || '',
        },
      };

      const response = await enrollSDH(payloadToSend);
  ...
  const getTrainingMajorPickerRules = (submitted: boolean) => {
    if (!submitted) {
      return [];
    }
    return [
      {
        validator: async (_: any, value: any) => {
          const majorCode = typeof value?.majorCode === 'number' ? value.majorCode : undefined;
          if (majorCode === undefined) {
            return Promise.reject(new Error('Vui lòng chọn Chuyên ngành tốt nghiệp'));
          }
          return Promise.resolve();
        },
      },
    ...
    <ConfigProvider locale={viVN}>
      ...
      >
        <div className={styles.formHeader}>
          ...
          />
          <h2 className={styles.headerText}>SAU ĐẠI HỌC</h2>
        </div>
        ...
        <h1 className={styles.section}>02. THÔNG TIN NGUYỆN VỌNG</h1>
        <Row gutter={[16, 16]}>
          <Col xs={24} md={12}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Chương trình đào tạo
                </Typography.Text>
              }
              name="educationType"
              rules={[{ required: true, message: 'Vui lòng chọn Chương trình' }]}
            >
              <Select placeholder="Thạc sĩ">
                <Select.Option value="M">Thạc sĩ</Select.Option>
                <Select.Option value="P">Tiến sĩ</Select.Option>
                <Select.Option value="R">Nghiên cứu sinh</Select.Option>
              </Select>
            </Form.Item>
          </Col>
          <Col xs={24} md={12}>
        ...
        {/* Các trường cho High Study Infor */}
        {educationType !== 'M' && (
          <>
            <h1 className={styles.section}>04. THÔNG TIN TRÌNH ĐỘ CAO HỌC</h1>
            <Row gutter={[16, 16]}>
              <Col xs={24} md={6}>
                <Form.Item
                  label={
                    <Typography.Text strong>
                      <span style={{ color: 'red' }}>* </span>Loại bằng
                    </Typography.Text>
                  }
                  name={['highStudyInfor', 'highStudyDegree']}
                  rules={[{ required: true, message: 'Vui lòng chọn Loại bằng' }]}
                >
                  <Select placeholder="Loại bằng">
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={<Typography.Text strong>Ngành tốt nghiệp</Typography.Text>}
                  name={['highStudyInfor', 'highStudyGraduateMajor']}
                  rules={
                    highStudyDegree === 1
                      ? [] // Không cần validate nếu là Thạc sĩ
                      : [{ required: true, message: 'Vui lòng nhập Ngành tốt nghiệp' }]
                  }
                >
                  <Input
                    placeholder="Luật Hình sự"
                    style={{ borderRadius: '8px' }}
                    disabled={highStudyDegree === 1}
                  />
                </Form.Item>
              </Col>
              <Col xs={24} md={6}>
                <Form.Item
                  label={<Typography.Text strong>Ngày ký</Typography.Text>}
                  name={['highStudyInfor', 'highStudyDate']}
                  rules={
                    highStudyDegree === 1
                      ? []
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  name={['highStudyInfor', 'highStudyTrainingMajor']}
                  rules={getTrainingMajorPickerRules(isFormSubmitted)}
                >
                  <TrainingMajorPicker majors={major} title />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item
                  ...
                  name={['highStudyInfor', 'highStudyUniversity']}
                  rules={[{ required: true, message: 'Vui lòng nhập Tên trường cấp' }, ...textRule]}
                >
                  <Input placeholder="Trường Đại học Luật TP.HCM" style={{ borderRadius: '8px' }} />
                </Form.Item>
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  ...
                  name={['highStudyInfor', 'highStudyDegreeFile']}
                  rules={[
                    { required: true, message: 'Vui lòng tải lên Minh chứng bằng tốt nghiệp' },
                  ]}
                >
                  <Upload
                    name="file"
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={
                    <Typography.Text strong>
                      <span style={{ color: 'red' }}>* </span>Bảng điểm
                    </Typography.Text>
                  }
                  name={['highStudyInfor', 'highStudyTranscript']}
                  rules={[{ required: true, message: 'Vui lòng tải lên Bảng điểm' }]}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={<Typography.Text strong>Đơn xin dự tuyển</Typography.Text>}
                  name={['highStudyInfor', 'highStudyApplication']}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={<Typography.Text strong>Lý lịch khoa học (có công chứng)</Typography.Text>}
                  name={['highStudyInfor', 'highStudyBackground']}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={
                    <Typography.Text strong>Minh chứng kinh nghiệm nghiên cứu</Typography.Text>
                  }
                  name={['highStudyInfor', 'highStudyReseachExperience']}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={<Typography.Text strong>Dự thảo đề cương nghiên cứu</Typography.Text>}
                  name={['highStudyInfor', 'highStudyReseachProposal']}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={<Typography.Text strong>Kế hoạch học tập</Typography.Text>}
                  name={['highStudyInfor', 'highStudyPlan']}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
                    ...
                    onChange={onUploadChange(['highStudyInfor', 'highStudyPlan'], setHighStudyPlan)}
                  >
                    <Button icon={<UploadOutlined />}>Tải lên</Button>
                  </Upload>
                </Form.Item>
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={<Typography.Text strong>Thư giới thiệu</Typography.Text>}
                  name={['highStudyInfor', 'highStudyRecommendationLetter']}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  label={<Typography.Text strong>Công văn cử đi học</Typography.Text>}
                  name={['highStudyInfor', 'highStudyLetterForStudy']}
                >
                  <Upload
                    name="file"
                    action={`${endpoint}/api/upload/file`}
              ...
              <p className={styles.noteText}>
                <span className={styles.highlight}>Lưu ý:</span> Vui lòng scan các File trên rõ ràng
                và định dạng bằng file PDF (.pdf)
              </p>
            </Row>
          </>
        )}
        ...
        <h1 className={styles.section}>{educationType !== 'M' ? '05' : '04'}. THÔNG TIN LIÊN HỆ</h1>
        <Form.Item
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} md={12}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Nơi công tác (Tỉnh/Thành phố)
                </Typography.Text>
              }
              name="workPlace"
              rules={[{ required: true, message: 'Vui lòng nhập Nơi công tác' }]}
            >
              <Input placeholder="Thành phố Hồ Chí Minh" style={{ borderRadius: '8px' }} />
            </Form.Item>
          </Col>
          <Col xs={24} md={24}>
            <Form.Item
        ...
        {/* Các trường chứng chỉ Tiếng Anh*/}
        <h1 className={styles.section}>
          {educationType !== 'M' ? '06' : '05'}. THÔNG TIN CHỨNG CHỈ TIẾNG ANH
        </h1>
        <Row gutter={[16, 16]}>
          <Col xs={24} md={12}>
            <Form.Item
          ...
          <Col xs={24} md={4}>
            <Form.Item
              ...
              rules={[{ required: true, message: 'Vui lòng nhập Tổng điểm' }, ...certificateMarkRule]}
            >
              <Input style={{ borderRadius: '8px' }} />
            </Form.Item>
          </Col>
          ...
          <Col xs={24} md={14}>
            ...
            <p className={styles.noteText}>
              <span className={styles.highlight}>Lưu ý:</span> Vui lòng scan "Minh chứng" rõ ràng và
              định dạng bằng file PDF (.pdf)
            </p>
          </Col>
        </Row>

        {/* Trường minh chứng đóng phí */}
        <h1 className={styles.section}>
          {educationType !== 'M' ? '07' : '06'}. MINH CHỨNG ĐÓNG PHÍ
        </h1>
        <Row gutter={[16, 16]}>
          <Col xs={24} md={12}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Minh chứng đóng phí
                </Typography.Text>
              }
              rules={[{ required: true, message: 'Vui lòng tải lên Minh chứng đóng phí' }]}
              name="feeFile"
            >
              <Upload
                name="file"
                ...
                action={`${endpoint}/api/upload/picture`}
                listType="text"
                fileList={highStudyFeeFile}
                onChange={(info) => {
                  const file = info.file;
                  const url = file?.response?.url;
                  setHighStudyFeeFile(info.fileList);
                  form.setFieldsValue({ feeFile: url });
                }}
                beforeUpload={beforeUploadAvatar}
              >
                <Button icon={<UploadOutlined />}>Tải lên</Button>
            ...
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Minh chứng & URL Minh chứng
                </Typography.Text>
              }
              name="feeFile"
              rules={[
                {
                  required: true,
                  message: 'URL minh chứng là bắt buộc. Vui lòng tải lên file .jpg',
                },
              ]}
            >
              <Input
                placeholder="URL file minh chứng"
                disabled
                style={{ color: 'rgba(0, 0, 0, 0.65)', borderRadius: '8px' }}
              />
            </Form.Item>
            ...
            <p className={styles.noteText}>
              <span className={styles.highlight}>Lưu ý:</span> Chỉ chấp nhận file ảnh định dạng JPG
              (.jpg)
            </p>
          </Col>
        </Row>
