...
import MajorPicker from '@/components/MajorPicker/MajorPicker';
...
const DaiHoc: React.FC = () => {
  ...
  const fetchAllProvincesFromLocalJson = async () => {
    try {
      const response = await axios.get('/cities_and_wards.json'); // "public/"
      if (response.data) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const payloadToSend = {
        ...
        trainingSystemType: 'VuaLamVuaHoc',
        educationType: 'DH',
        aspirationMajor: values.userMajor?.majorCode,
        aspirationExamGroup: values.userMajor?.subjectGroupCode,
        aspirationAdmissionMethod: values.aspirationAdmissionMethod,
        aspirationSubject1Score: parseFloat(values.aspirationSubject1Score),
        aspirationSubject2Score: parseFloat(values.aspirationSubject2Score),
        aspirationSubject3Score: parseFloat(values.aspirationSubject3Score),
      ...
      const response = await enrollDH(payloadToSend);
  ...
  const getMajorPickerRules = (submitted: boolean) => {
    if (!submitted) {
      return [];
    }
    return [
      {
        validator: async (_: any, value: any) => {
          ...
          const subjectGroupCode =
            typeof value?.subjectGroupCode === 'string' && value.subjectGroupCode !== ''
              ? value.subjectGroupCode
              : undefined;
          if (majorCode === undefined) {
            return Promise.reject(new Error('Vui lòng chọn Ngành'));
          }
          if (subjectGroupCode === undefined) {
            return Promise.reject(new Error('Vui lòng chọn Tổ hợp môn'));
          }
          return Promise.resolve();
        },
      },
    ...
    <ConfigProvider locale={viVN}>
      ...
      >
        <div className={styles.formHeader}>
          ...
          />
          <h2 className={styles.headerText}>HỆ VỪA LÀM VỪA HỌC - ĐẠI HỌC</h2>
        </div>
        ...
        {/* Các trường demo cho HighSchoolInfor */}
        ...
        {/* Các trường demo cho ContactInfor */}
        <h1 className={styles.section}>03. THÔNG TIN LIÊN HỆ</h1>
        <Form.Item
        ...
        <h1 className={styles.section}>04. THÔNG TIN CHỨNG CHỈ</h1>
        <Row gutter={[16, 16]}>
          <Col xs={24} md={24}>
        ...
        {/* Các trường demo cho Aspiration */}
        <h1 className={styles.section}>05. THÔNG TIN NGUYỆN VỌNG</h1>
        <Form.Item name="userMajor" rules={getMajorPickerRules(isFormSubmitted)}>
          <MajorPicker majors={major} title />
        </Form.Item>
        <Row gutter={[16, 16]}>
          <Col xs={24} md={6}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Phương thức xét tuyển
                </Typography.Text>
              }
              name="aspirationAdmissionMethod"
              rules={[{ required: true, message: 'Vui lòng chọn Phương thức' }]}
            >
              <Select placeholder="Học bạ">
          ...
          <Col xs={24} md={6}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Điểm môn 1
                </Typography.Text>
              }
              name="aspirationSubject1Score"
              rules={[{ required: true, message: 'Vui lòng nhập Điểm môn 1' }, ...markRule]}
            >
              <Input placeholder="8.0" style={{ borderRadius: '8px' }} />
            </Form.Item>
          </Col>
          <Col xs={24} md={6}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Điểm môn 2
                </Typography.Text>
              }
              name="aspirationSubject2Score"
              rules={[{ required: true, message: 'Vui lòng nhập Điểm môn 2' }, ...markRule]}
            >
              <Input placeholder="7.5" style={{ borderRadius: '8px' }} />
            </Form.Item>
          </Col>
          <Col xs={24} md={6}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Điểm môn 3
                </Typography.Text>
              }
              name="aspirationSubject3Score"
              rules={[{ required: true, message: 'Vui lòng nhập Điểm môn 3' }, ...markRule]}
            >
              <Input placeholder="9.0" style={{ borderRadius: '8px' }} />
            </Form.Item>
          </Col>
        </Row>
