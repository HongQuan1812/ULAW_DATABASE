...
const LienThong: React.FC = () => {
  const trainingSystemType = 'VuaLamVuaHoc';
  const educationType = 'lienthong';
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      const response = await enrollLT(payloadToSend);
    ...
    <ConfigProvider locale={viVN}>
      ...
      >
        <div className={styles.formHeader}>
          ...
          />
          <h2 className={styles.headerText}>HỆ VỪA LÀM VỪA HỌC - LIÊN THÔNG</h2>
        </div>
        ...
        <h1 className={styles.section}>03. THÔNG TIN TRÌNH ĐỘ TRUNG CẤP/CAO ĐẲNG</h1>
        <Row gutter={[16, 16]}>
          <Col xs={24} md={12}>
            <Form.Item
              label={
          ...
          <Col xs={24} md={4}>
            <Form.Item
              label={
                <Typography.Text strong>
                  <span style={{ color: 'red' }}>* </span>Bậc tốt nghiệp
                </Typography.Text>
              }
              name={['universityInfor', 'universityGraduateDegree']}
              rules={[{ required: true, message: 'Vui lòng chọn Bậc tốt nghiệp' }]}
            >
              <Select placeholder="Trung cấp">
                <Select.Option value="TC">Trung cấp</Select.Option>
                <Select.Option value="CD">Cao đẳng</Select.Option>
              </Select>
            </Form.Item>
          </Col>
          <Col xs={24} md={18}>
            ...
            >
              <Input
                placeholder="Trường Cao đẳng Kỹ thuật Cao Thắng"
